...
class UISRNN:
    ...
    def _update_beam_state(self, beam_state, look_ahead_seq, cluster_seq):
        ...
        for sub_idx, cluster in enumerate(cluster_seq):
            if cluster > len(new_beam_state.mean_set):  # invalid trace
                ...
                init_input = autograd.Variable(torch.zeros(self.observation_dim)).unsqueeze(0).unsqueeze(0).to(self.device)
    ...
    def predict_single(self, test_sequence, args):
        ...
        test_sequence = autograd.Variable(torch.from_numpy(test_sequence).float()).to(self.device)
        # bookkeeping for beam search
        beam_set = [BeamState()]
        total_iter = args.test_iteration * test_sequence_length
        for num_iter in np.arange(0, args.test_iteration * test_sequence_length,
                                  args.look_ahead):
            print('num_iter:', num_iter, 'total iter:', total_iter, "%.1f%%" % (num_iter / total_iter * 100))
